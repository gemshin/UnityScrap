Shader "BGShader/Transparent/Alpha" {
Properties 
	{
		_MainTex("MainTex", 2D) = "white" {}
		_Color("Main Color", Color) = (0.5,0.5,0.5,1)
		_GlowTex("GlowTex", 2D) = "black" {}
		_GlowAlphaTex("GlowAlphaTex", 2D) = "black" {}
		...
		_CtrlAlpha("CtrlAlpha", Range(0.0,1.0)) = 1
	...
	SubShader 
	{
		...
		float _CtrlAlpha;
			...
			void surf (Input IN, inout EditorSurfaceOutput o) {
				...
				float4 Tex2D3=tex2D(_GlowAlphaTex,(IN.uv_GlowAlphaTex.xyxy).xy);
				float4 Multiply8=float4( 10,10,10,10 ) * Tex2D3;
				float4 Min0=min(Multiply8,float4( 1,1,1,1 ));
				float4 Invert0= float4(1.0, 1.0, 1.0, 1.0) - Min0;
				float4 Multiply1=Add2 * Invert0;
				float4 Add3 = (IN.uv_SpecTex.xyxy) + _ScrollRange2;
				float4 Tex2D2=tex2D(_SpecTex,Add3.xy);
				float4 Multiply6=Invert0 * Tex2D2;
				float4 Multiply10=Multiply1 * Multiply6;
				...
				float4 Multiply2=Tex2D1 * Tex2D3;
				...
				float4 Multiply9=Multiply6 * _SpecColors;
				float4 Multiply11=Tex2D0.aaaa * _CtrlAlpha.xxxx;
